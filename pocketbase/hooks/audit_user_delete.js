onRecordDeleteRequest((e) => {
  ...
  if (recordId) {
    try {
      ...
      auditRecord.set('action', 'DELETE_USER')
      auditRecord.set('resource', 'users')
      auditRecord.set('resource_id', recordId)
      auditRecord.set('details', 'Usuário excluído' + (userName ? ': ' + userName : ''))
      $app.save(auditRecord)
    } catch (err) {
      $app
        .logger()
        .error('Failed to create audit log for user deletion', 'error', (err && err.message) || '')
    }
  }
}, 'users')
