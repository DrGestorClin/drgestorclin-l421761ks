onRecordUpdateRequest((e) => {
  ...
  if (recordId) {
    try {
      ...
      auditRecord.set('action', 'UPDATE_USER')
      auditRecord.set('resource', 'users')
      auditRecord.set('resource_id', recordId)
      auditRecord.set('details', 'Usuário atualizado' + (userName ? ': ' + userName : ''))
      $app.save(auditRecord)
    } catch (err) {
      $app
        .logger()
        .error('Failed to create audit log for user update', 'error', (err && err.message) || '')
    }
  }
}, 'users')
